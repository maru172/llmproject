    ...
    def process_news_category(driver, df, category, sequence, prev_news):
        ...
        for i, url in enumerate(new_urls):
            try:
                ...
                # SQLite 저장
                for db_path in [
                    f"/home/llmproject/Desktop/Transformer/llmproject2/databases/{category}.db",
                    f"{folder_path}/{category}.db"
                ]:
                    SaveDB.save_news_to_db(
                        category, main_page_urls[i], main_page_titles[i], main_page_press[i], _news_author, _news_date_time,
                        _image_url, _news_original_text, _news_summary, _news_caption, _generated_caption, db_path, category
                    )
